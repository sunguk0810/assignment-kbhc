...
### 2.1. 사용자 계정 (`users`)
> **설계 의도**: 사용자의 고유 식별자로 자동 증가형 Integer 대신 **UUID**를 채택했습니다. 이는 향후 MSA 환경이나 데이터베이스 샤딩 시 ID 충돌을 방지하고, 외부 노출 시 보안성을 높이기 위함입니다.
...
> **설계 의도**: 사용자의 인증 정보(`users`)와 신체 정보(`user_profiles`)를 분리하여 1:1 관계로 설계했습니다. 이는 자주 조회되는 인증 정보 테이블을 경량화하고, 민감한 신체 정보를 별도로 관리하기 위함입니다.
...
> **설계 의도**: 통계 API 호출 시마다 수만 건의 `infos` 데이터를 `GROUP BY` 하는 것은 비효율적입니다. 따라서 데이터 수집 시점(Kafka Consumer)에 실시간으로 일별 통계를 미리 계산(Pre-aggregation)하여 저장하는 **데이터 마트(Data Mart)** 성격의 테이블입니다.
...
2.  **Users ↔ Logs/Infos/Summaries (1:N)**:
    ...
    *   모든 하위 테이블은 `record_key`를 파티셔닝 키(Sharding Key) 후보로 사용할 수 있도록 포함하고 있습니다.

3.  **Logs ⇢ Infos**:
    *   `health_measure_infos`는 `health_measure_logs`의 ID를 가지고 있습니다.
    *   이는 **"어떤 원본 로그로부터 이 상세 데이터가 파생되었는가?"** 를 추적(Tracing)하기 위함입니다. 다만, 로그 데이터가 아카이빙(삭제)되더라도 상세 데이터는 유지되어야 하므로 물리적 FK 제약조건은 설정하지 않았습니다.
